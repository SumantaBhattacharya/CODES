...
const ProductDts = () => {
    ...
    const {dataProduct: product, status, error,} = useSelector((state) => {
    ...
    const handleAdd = (product) => {
        // When you dispatch(add(product)), 
        // you are sending a message from your component saying "User wants to add this product." You export individual actions so components can trigger them.
    ...
    if (status === "error") {
        return (
            ...
            style={{
                ...
                <h1 
                style={{ color: '#ef4444' }}>Error: {error}</h1>
                <button
    ...
    // Add this after loading and error checks
    if (!product) {
        return (
            <div style={{
                display: 'flex',
                ...
                style={{backgroundColor: '#1f2937',
                    ...
                    <div style={{padding: isMobile ? '1.5rem' : '2rem'}}>
                        ...
                        {/* <button> is natively keyboard-accessible */}
                            <button 
                            style={{backgroundColor: '#1f2937',color: '#fff',border: '1px solid #374151', padding: '12px 28px',borderRadius: '6px',cursor: 'pointer',fontSize: '1rem',fontWeight: '600',
                                width: isMobile ? '100%' : 'auto',textAlign: 'center',boxSizing: 'border-box',transition: 'all 0.2s ease'}}
                            // tabIndex={0}
